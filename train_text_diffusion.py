...
def main(args):
    ...
    model = DiffusionTransformer(
        ...
        use_latent_kv=args.use_latent_kv
    ).cuda()
    ...
    if args.eval:
        ...
        if trainer.diffusion.diffusion_model.seq2seq:
            trainer.sample_seq2seq()
            trainer.sample_seq2seq(split='train')
            trainer.sample_seq2seq(cls_free_guidance=2.0)
        elif args.infill:
            trainer.sample(infill = args.infill)
        elif args.outfill:
            trainer.sample(outfill = args.outfill)
        else:
            trainer.sample()
    ...
    if args.eval_test:
        ...
        trainer.load(args.resume_dir)
        if trainer.diffusion.diffusion_model.seq2seq:
            trainer.sample_seq2seq(split='test')
            trainer.sample_seq2seq(split='test', cls_free_guidance=2.0)
        else:
    ...
    if args.inference:
        trainer.load(args.resume_dir)
        if args.infill:
            trainer.inference(infill = args.infill)
        elif args.outfill:
            trainer.inference(outfill = args.outfill)
        else:
            trainer.inference()
        return
    ...
    if args.only_decoder:
        trainer.load(args.resume_dir, only_decoder=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--adam_beta2", type=float, default=0.980)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["beta_linear", "cosine", "simple_linear", 'tand', 'laplace', None],
        help=(
            "Which noise schedule to use."
        ),
    )
    ...
    parser.add_argument("--max_seq_len", type=int, default=419)
    parser.add_argument("--context_max_seq_len", type=int, default=107)
    ...
    parser.add_argument(
        ...
        type=str,
        default='fixed',
        choices=["fixed", "theory", "hybrid"],
        help=(
            "Which conditioned sampling way to use."
        ),
    )
    ...
    parser.add_argument("--rotary_pos_emb", action="store_true", default=False)
    parser.add_argument("--use_latent_kv", action="store_true", default=False)
    ...
    parser.add_argument("--infill", action="store_true", default=False)
    parser.add_argument("--outfill", action="store_true", default=False)
    ...
    parser.add_argument("--only_decoder", action="store_true", default=False)
        
    args = parser.parse_args()
    assert not (args.eval and args.resume_training)
    assert not (args.inference and args.resume_training)
    if args.eval or args.resume_training or args.inference:
        assert args.resume_dir is not None

    if args.eval or args.resume_training or args.eval_test or args.inference:
        with open(os.path.join(args.resume_dir, 'args.json'), 'rt') as f:
            saved_args = json.load(f)
        args_dict = vars(args)
        # Hold out sampling/evaluation parameters
        heldout_params = {'wandb_name', 'output_dir', 'resume_dir', 'eval', 'eval_test', 'inference', 'infill', 'outfill', 'num_samples', 'sampling_timesteps',
                          'sampling_schedule', 'seq2seq_candidates', 'scale', 'sampler', 'method', 'resume_training', 'only_decoder'}
