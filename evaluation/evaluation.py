...
def compute_diversity(args, all_texts_list):
    ngram_range = [2,3,4]
    ...
    for n in ngram_range:
        ...
        try:
            ...
            print(f'ngram_counts[{n}] failed due to {type(error).__name__} - {error}')
...
def compute_memorization(args, all_texts_list, human_references, n=4):
    ...
    try:
        memorization = duplicate/total
    except Exception as error:
        print(f'memorization failed due to {type(error).__name__} - {error}')
        memorization = None
