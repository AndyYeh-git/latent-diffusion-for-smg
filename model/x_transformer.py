...
class DataDependentAlibi(nn.Module):
...
class PerRowDataDependentAlibi(nn.Module):
...
class Attention(nn.Module):
    def __init__(
        ...
        selective = False,
        custom_attn_fn: Callable | None = None,
        hybrid_module: nn.Module | None = None,
...
class AttentionLayers(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.skip_combines = nn.ModuleList([])
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type == 'a':
                ...
                layer = Attention(dim, heads = heads, causal = causal, qkv_receive_diff_residuals = qkv_receive_diff_residuals, learned_value_residual_mix = self_attn_learned_value_residual,
                                  use_latent_kv = use_latent_kv, dim_latent_kv = 4 * DEFAULT_DIM_HEAD if use_latent_kv else None, **attn_kwargs)
                is_first_self_attn = False
            elif layer_type == 'c':
                layer = Attention(dim, heads = heads, use_latent_kv = use_latent_kv, dim_latent_kv = 4 * DEFAULT_DIM_HEAD if use_latent_kv else None,
                                  **{**attn_kwargs, **cross_attn_kwargs})
