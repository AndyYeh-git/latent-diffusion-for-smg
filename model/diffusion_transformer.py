...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        self.seq2seq_context_dim = seq2seq_context_dim
        ...
        if not rotary_pos_emb:
            self.pos_emb = AbsolutePositionalEmbedding(tx_dim, max_seq_len, l2norm_embed = True)
    ...
    def forward(self, x, mask, time, x_self_cond = None, class_id = None, seq2seq_cond = None, seq2seq_mask = None):
        ...
        if self.cross:
            ...
            #x = self.encoder(inputs_embeds=tx_input, attention_mask=mask, encoder_hidden_states=context, encoder_attention_mask=context_mask).last_hidden_state

        else:
            x = self.encoder(tx_input, mask=mask, time_emb=time_emb)
            #x = self.encoder(inputs_embeds=tx_input, attention_mask=mask).last_hidden_state
